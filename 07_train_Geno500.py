...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler("log500.txt"),
...
tokenizer = GPT2TokenizerFast.from_pretrained("tokenizer500")
...
training_args = TrainingArguments(
    ...
    fp16=True,  # If you want to use 16-bit precision, set this to True
    fp16_opt_level='O1',  # This configures the level of FP16 autocasting
    dataloader_drop_last=True,  # Drop the last incomplete batch if it is not divisible by the number of GPUs
)


# Check if the tokenized datasets exist
if not os.path.exists("tokenized_train_dataset5PT") or not os.path.exists("tokenized_test_dataset5PT"):
    ...
    train_dataset.save_to_disk("tokenized_train_dataset5PT")
    test_dataset.save_to_disk("tokenized_test_dataset5PT")
else:
    # Load the tokenized datasets
    train_dataset = load_from_disk("tokenized_train_dataset5PT")
    test_dataset = load_from_disk("tokenized_test_dataset5PT")
...
if checkpoints:
    ...
    trainer.train()
# Save the model
model.save_pretrained("Geno_GPT2_500")
